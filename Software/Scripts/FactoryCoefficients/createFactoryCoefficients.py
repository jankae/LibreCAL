...
if args.directory:
    ...
    # Opens
    for o in Opens:
        filename = "P"+str(o)+"_OPEN.s1p"
        file = open(args.directory + "/" + filename, "w")
        file.write("! Generated by the LibreCAL factory coefficient script\n")
        file.write("! "+datetime.now().isoformat()+"\n")
        file.write("# GHz S RI R 50.0\n")
        for point in Opens[o]:
            freq = point[0] / 1000000000.0
            S = point[1]
            file.write(str(freq)+" "+str(S.real)+" "+str(S.imag)+"\n")
        file.close()
    # Shorts
    for s in Shorts:
        filename = "P" + str(s) + "_SHORT.s1p"
        file = open(args.directory + "/" + filename, "w")
        file.write("! Generated by the LibreCAL factory coefficient script\n")
        file.write("! " + datetime.now().isoformat() + "\n")
        file.write("# GHz S RI R 50.0\n")
        for point in Shorts[s]:
            freq = point[0] / 1000000000.0
            S = point[1]
            file.write(str(freq) + " " + str(S.real) + " " + str(S.imag) + "\n")
        file.close()
    # Loads
    for load in Loads:
        filename = "P" + str(load) + "_LOAD.s1p"
        file = open(args.directory + "/" + filename, "w")
        file.write("! Generated by the LibreCAL factory coefficient script\n")
        file.write("! " + datetime.now().isoformat() + "\n")
        file.write("# GHz S RI R 50.0\n")
        for point in Loads[load]:
            freq = point[0] / 1000000000.0
            S = point[1]
            file.write(str(freq) + " " + str(S.real) + " " + str(S.imag) + "\n")
        file.close()
    # Throughs
    for t in Throughs:
        filename = "P" + str(t) + "_THROUGH.s2p"
        file = open(args.directory + "/" + filename, "w")
        file.write("! Generated by the LibreCAL factory coefficient script\n")
        file.write("! " + datetime.now().isoformat() + "\n")
        file.write("# GHz S RI R 50.0\n")
        through = Throughs[t]
        for i in range(len(through["S11"])):
            freq = through["S11"][i][0] / 1000000000.0
            S11 = through["S11"][i][1]
            S12 = through["S12"][i][1]
            S21 = through["S21"][i][1]
            S22 = through["S22"][i][1]
            file.write(str(freq)
                +" "+str(S11.real)+" "+str(S11.imag)
                +" "+str(S21.real)+" "+str(S21.imag)
                +" "+str(S12.real)+" "+str(S12.imag)
                +" "+str(S22.real)+" "+str(S22.imag) + "\n")
        file.close()
